...
TEST_CASE("test select construction", "[all]") {
    ...
    REQUIRE(SelectRequest::Select("*")
        ...
        .Where("sex > 3")
        .Limit(10)
        .Finalize().ToString() == "SELECT * FROM sosal WHERE sex > 3 LIMIT 10;");
}
...
TEST_CASE("test delete construction", "[all]") {
    ...
    REQUIRE(DeleteRequest::Delete()
        ...
        .Where("sex > 3")
        .Limit(10)
        .Finalize().ToString() == "DELETE FROM sosal WHERE sex > 3 LIMIT 10;");
}

TEST_CASE("test insert construction", "[all]") {
    REQUIRE(InsertRequest::Insert("mom").ToString() == "INSERT INTO mom");
    REQUIRE_THROWS(InsertRequest::Insert("mom").Values(std::vector<std::string>{"1", "ssss", "sfdkf"})
        .Finalize().ToString()
        == "INSERT INTO mom VALUES(1,ssss,sfdkf);");
